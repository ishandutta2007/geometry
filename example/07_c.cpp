...
void read_points(std::string const& filename, Geometry& geometry)
{
    std::ifstream cpp_file(filename.c_str());
    if (cpp_file.is_open())
    {
...
int main()
{
    ...
    read_wkt<point_type>("data/nlcities.wkt", cities, box);

    // Read the road network
    typedef boost::tuple<line_type, std::string> road_type;
    std::vector<road_type> roads;
    read_wkt<line_type>("data/nlroads.wkt", roads, box);
...
std::cout << "Graph build time: " << std::setprecision(6) << elapsed << " ms" << std::endl;
    }
    ...
    line_type ttp;
    read_points("data/adam-hoorn.wkt", ttp);
    ...
    BOOST_FOREACH(city_type const& city1, cities)
    {
        ...
        auto finish = std::chrono::steady_clock::now();
double elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(finish - start).count();
std::cout << std::setprecision(6) << elapsed << " ms" << std::endl;
    ...
    mapper.map(ttp, "stroke:rgb(0,0,255);stroke-width:6;opacity:0.5");
